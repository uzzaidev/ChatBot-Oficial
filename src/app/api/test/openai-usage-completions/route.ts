...
/**
 * Test OpenAI Usage Completions API endpoint
 * GET /api/test/openai-usage-completions
 */
export async function GET(request: NextRequest) {
  try {
    ...
    console.log("[Test Usage Completions API] Fetching from:", url);
    ...
    if (!response.ok) {
      return NextResponse.json({
        ...
        error: data,
        message: "❌ Usage Completions API - SEM PERMISSÃO",
      });
    }
    ...
    return NextResponse.json({
      ...
      status: response.status,
      data,
      message: "✅ Usage Completions API - TEM PERMISSÃO!",
    });
  } catch (error) {
