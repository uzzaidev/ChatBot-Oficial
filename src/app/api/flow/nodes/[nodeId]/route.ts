  ...
  try {
    ...
    if (nodeId === "generate_response") {
      ...
      const { data: clientData } = await supabase
        .from("clients")
        .select(
          "system_prompt, formatter_prompt, openai_model, groq_model, primary_model_provider, settings",
        )
        .eq("id", clientId)
        .single();
      ...
      // Special handling for TTS - fetch from clients table columns (same as /settings/tts)
      const { data: clientData } = await supabase
        .from("clients")
        .select("tts_provider, tts_voice, tts_speed, tts_auto_offer")
        .eq("id", clientId)
        .single();

      if (clientData) {
        config.provider = clientData.tts_provider || "openai";
        config.voice = clientData.tts_voice || "alloy";
        config.speed = clientData.tts_speed || 1.0;
        config.auto_offer = clientData.tts_auto_offer ?? true;
      }
    } else {
      ...
      if (configKey) {
        ...
        if (configData && configData.config_value !== null) {
          ...
          if (
            ...
            !Array.isArray(configData.config_value)
          ) {
      ...
      if (relatedConfigKeys.length > 0) {
        const { data: relatedConfigs, error: relatedFetchError } =
          await supabase
            .from("bot_configurations")
        ...
        if (relatedConfigs && relatedConfigs.length > 0) {
          ...
          relatedConfigs.forEach((item) => {
            if (item.config_value !== null) {
              if (
                typeof item.config_value === "object" &&
    ...
    if (config) {
      if (nodeId === "generate_response") {
        ...
        if (config.system_prompt !== undefined) {
          updateData.system_prompt = config.system_prompt;
        }
        if (config.formatter_prompt !== undefined) {
          updateData.formatter_prompt = config.formatter_prompt;
        }
        if (config.openai_model !== undefined) {
          updateData.openai_model = config.openai_model;
        }
        if (config.groq_model !== undefined) {
          updateData.groq_model = config.groq_model;
        }
        if (config.primary_model_provider !== undefined) {
          updateData.primary_model_provider = config.primary_model_provider;
        }
        ...
        if (
          ...
          if (config.temperature !== undefined) {
            newSettings.temperature = config.temperature;
          }
          if (config.max_tokens !== undefined) {
            newSettings.max_tokens = config.max_tokens;
          }
        ...
        if (config.provider !== undefined) {
          updateData.tts_provider = config.provider;
        }
        if (config.voice !== undefined) updateData.tts_voice = config.voice;
        if (config.speed !== undefined) updateData.tts_speed = config.speed;
        if (config.auto_offer !== undefined) {
          updateData.tts_auto_offer = config.auto_offer;
        }
...
function getDefaultConfig(nodeId: string): Record<string, any> {
  const defaults: Record<string, Record<string, any>> = {
    ...
    classify_intent: {
      ...
      prompt: {
        system:
          "Você é um classificador de intenções. Analise a mensagem do usuário e identifique a intenção principal.",
        temperature: 0.1,
        max_tokens: 10,
      },
      intents: [
        {
          key: "saudacao",
          label: "Saudação",
          description: "Cliente cumprimentando ou iniciando conversa",
        },
        {
          key: "duvida_tecnica",
          label: "Dúvida Técnica",
          description: "Perguntas sobre como algo funciona",
        },
        {
          key: "orcamento",
          label: "Orçamento",
          description: "Solicitação de preço ou cotação",
        },
        {
          key: "agendamento",
          label: "Agendamento",
          description: "Marcar reunião ou horário",
        },
        {
          key: "reclamacao",
          label: "Reclamação",
          description: "Insatisfação ou problema",
        },
        {
          key: "agradecimento",
          label: "Agradecimento",
          description: "Gratidão pelo atendimento",
        },
        {
          key: "despedida",
          label: "Despedida",
          description: "Finalização de conversa",
        },
        {
          key: "transferencia",
          label: "Transferência",
          description: "Quer falar com humano",
        },
        {
          key: "outro",
          label: "Outro",
          description: "Intenção não identificada",
        },
      ],
    },
    detect_repetition: {
