...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      ...
      return NextResponse.json({ error: "Failed to fetch configuration" }, {
        status: 500,
      });
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const hasExistingGatewayKey = !!currentConfig.gateway_api_key_secret_id;

    // Validate gateway key only if provided OR required (first-time setup)
    if (!hasExistingGatewayKey && !body.gatewayApiKey) {
      return NextResponse.json(
        { error: "Gateway API Key is required for first-time setup" },
        { status: 400 },
      );
    }
    ...
    const createOrUpdateSecret = async (
      ...
      try {
        ...
        if (error) {
          console.error(
            `[AI Gateway Setup] Error creating secret ${secretName}:`,
            error,
          );
        ...
        console.log(
          `[AI Gateway Setup] Created/retrieved secret ${secretName}: ${secretId}`,
        );
        return secretId;
      } catch (error: any) {
        console.error(
          `[AI Gateway Setup] Exception creating secret ${secretName}:`,
          error,
        );
        throw error;
      }
    ...
    // 1. Gateway API Key (optional if already configured)
    if (body.gatewayApiKey) {
      try {
        secretIds.gateway = await createOrUpdateSecret(
          body.gatewayApiKey,
        ...
        return NextResponse.json({
          error: "Failed to save gateway key to Vault",
        }, { status: 500 });
      }
    }
    ...
    if (body.openaiApiKey) {
      try {
        secretIds.openai = await createOrUpdateSecret(
          body.openaiApiKey,
          "shared_openai_api_key",
        );
      } catch (error) {
        console.warn(
          "[AI Gateway Setup] Failed to save OpenAI key, continuing...",
        );
      }
    }
    ...
    if (body.groqApiKey) {
      try {
        secretIds.groq = await createOrUpdateSecret(
          body.groqApiKey,
          "shared_groq_api_key",
        );
      } catch (error) {
        console.warn(
          "[AI Gateway Setup] Failed to save Groq key, continuing...",
        );
      }
    }
    ...
    if (body.anthropicApiKey) {
      try {
        ...
        console.warn(
          "[AI Gateway Setup] Failed to save Anthropic key, continuing...",
        );
      }
    }
    ...
    if (body.googleApiKey) {
      try {
        secretIds.google = await createOrUpdateSecret(
          body.googleApiKey,
          "shared_google_api_key",
        );
      } catch (error) {
        console.warn(
          "[AI Gateway Setup] Failed to save Google key, continuing...",
        );
      }
    }
    ...
    if (secretIds.gateway) {
      updateData.gateway_api_key_secret_id = secretIds.gateway;
    }

    if (secretIds.openai) {
      updateData.openai_api_key_secret_id = secretIds.openai;
    }
    if (secretIds.groq) updateData.groq_api_key_secret_id = secretIds.groq;
    if (secretIds.anthropic) {
      updateData.anthropic_api_key_secret_id = secretIds.anthropic;
    }
    if (secretIds.google) {
      updateData.google_api_key_secret_id = secretIds.google;
    }
    ...
    if (body.defaultFallbackChain !== undefined) {
    ...
    if (updateError) {
      ...
      return NextResponse.json({ error: "Failed to update configuration" }, {
        status: 500,
      });
    }
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    // Fetch current config
    const { data: config } = await supabase.from("shared_gateway_config")
      .select("*").single();

    if (!config) {
      return NextResponse.json({ error: "No configuration to delete" }, {
        status: 404,
      });
    }
    ...
    if (error) {
      ...
      return NextResponse.json({ error: "Failed to clear configuration" }, {
        status: 500,
      });
    }
