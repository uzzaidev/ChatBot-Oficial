...
export async function GET(request: NextRequest) {
  try {
    const debugEnabled = process.env.UNIFIED_ANALYTICS_DEBUG === "true";
    ...
    debugLog(
      "[Unified Analytics] Filters - Period:",
      period,
      "Client:",
      filterClientId,
      "API Type:",
    ...
    debugLog(
      "[Unified Analytics] Using",
      isAdmin ? "SERVICE CLIENT (bypass RLS)" : "REGULAR CLIENT (with RLS)",
    );
    ...
    const targetClientId = isAdmin && !filterClientId
      ? null
      : (filterClientId || userClientId);

    debugLog(
      "[Unified Analytics] Target Client ID:",
      targetClientId || "ALL CLIENTS",
      "Is Admin:",
      isAdmin,
    );
    ...
    if (!isAdmin) {
      ...
      debugLog(
        "[Unified Analytics] Filtering gateway by client_id (tenant):",
        userClientId,
      );
      ...
      debugLog(
        "[Unified Analytics] Filtering gateway by client_id (admin filter):",
        filterClientId,
      );
      ...
      debugLog(
        "[Unified Analytics] Fetching gateway logs for ALL clients (admin, no filter)",
      );
    }
    ...
    const { data: gatewayLogs, error: gatewayError } = await gatewayQuery.order(
      "created_at",
      { ascending: false },
    );

    debugLog(
      "[Unified Analytics] Gateway logs fetched:",
    ...
    if (!isAdmin) {
      debugLog(
        "[Unified Analytics] Filtering usage_logs by client_id:",
        userClientId,
      );
      usageQuery = usageQuery.eq("client_id", userClientId);
    } else if (filterClientId) {
      debugLog(
        "[Unified Analytics] Filtering usage_logs by admin filter client_id:",
        filterClientId,
      );
      usageQuery = usageQuery.eq("client_id", filterClientId);
    } else {
      debugLog(
        "[Unified Analytics] Fetching ALL usage_logs (admin mode, no filter)",
      );
    }
    ...
    const { data: usageLogs, error: usageError } = await usageQuery.order(
      "created_at",
      { ascending: false },
    );

    debugLog(
      "[Unified Analytics] Legacy usage_logs fetched:",
      usageLogs?.length || 0,
      "Error:",
      usageError?.message,
    );
    ...
    debugLog(
      "[Unified Analytics] Top conversations fetched:",
    ...
    const gatewayCostBRL = gatewayLogs?.reduce((sum, log) =>
      sum + (log.cost_brl || 0), 0) || 0;
    const gatewayCostUSD = gatewayLogs?.reduce((sum, log) =>
      sum + (log.cost_usd || 0), 0) || 0;

    const gatewayTotalTokens = gatewayLogs?.reduce((sum, log) =>
      sum + (log.total_tokens || 0), 0) || 0;
    const gatewayTotalUnits = gatewayLogs?.reduce((sum, log) =>
      sum + ((log.input_units || 0) + (log.output_units || 0)), 0) || 0;

    const cachedRequests = gatewayLogs?.filter((log) =>
      log.was_cached
    ).length || 0;
    const cacheHitRate = gatewayRequestsCount > 0
      ? (cachedRequests / gatewayRequestsCount) * 100
      : 0;

    const totalLatency = gatewayLogs?.reduce((sum, log) =>
      sum + (log.latency_ms || 0), 0) || 0;
    const averageLatencyMs = gatewayRequestsCount > 0
      ? Math.round(totalLatency / gatewayRequestsCount)
      : 0;

    // By API Type
    const apiTypeUsage: Record<
      string,
      { requests: number; tokens: number; units: number; costBRL: number }
    > = {};

    for (const log of gatewayLogs || []) {
      const apiType = log.api_type || "chat";
      if (!apiTypeUsage[apiType]) {
        apiTypeUsage[apiType] = {
          requests: 0,
          tokens: 0,
          units: 0,
          costBRL: 0,
      ...
      apiTypeUsage[apiType].tokens += log.total_tokens || 0;
      apiTypeUsage[apiType].units += (log.input_units || 0) +
        (log.output_units || 0);
    ...
    const byApiType = Object.entries(apiTypeUsage).map(([apiType, usage]) => ({
      ...
      percentage: gatewayRequestsCount > 0
        ? (usage.requests / gatewayRequestsCount) * 100
        : 0,
    }));
    ...
    const byProvider = Object.entries(providerUsage).map((
      [provider, usage],
    ) => ({
      provider,
      requests: usage.requests,
    ...
    const chatbotMessagesCount = usageLogs?.reduce((sum, log) =>
      sum + (log.messages_sent || 0), 0) || 0;
    const chatbotCostUSD = usageLogs?.reduce((sum, log) =>
      sum + (log.cost_usd || 0), 0) || 0;
    const uniqueConversations = new Set(
      usageLogs?.map((log) => log.conversation_id),
    ).size;

    const modelUsage: Record<
      string,
      { messages: number; tokens: number; costUSD: number }
    > = {};
    ...
    if (isAdmin) {
      ...
      debugLog(
        "[Unified Analytics] Clients list for dropdown:",
        clientsList.length,
        "clients",
      );
    }
    ...
    if (isAdmin && !filterClientId) {
      ...
      const clientUsage: Record<
        string,
        { requests: number; costBRL: number; costUSD: number }
      > = {};
      ...
      debugLog(
        "[Unified Analytics] Client aggregation:",
        Object.keys(clientUsage).length,
        "clients found",
      );
      ...
      byClient = Object.entries(clientUsage)
        .map(([clientId, usage]) => ({
          ...
          percentage: totalRequests > 0
            ? (usage.requests / totalRequests) * 100
            : 0,
        }))
        .sort((a, b) =>
      ...
      debugLog(
        "[Unified Analytics] By Client:",
        byClient.map((c) =>
          `${c.clientName}: ${c.requests} requests (${
    ...
    debugLog("[Unified Analytics] Summary:");
    debugLog(
      "  - Gateway logs:",
      gatewayRequestsCount,
      "requests, R$",
      gatewayCostBRL.toFixed(2),
    );
    debugLog(
      "  - Legacy logs:",
      usageLogs?.length || 0,
      ...
      "USD:",
      chatbotCostUSD.toFixed(2),
    );
    debugLog(
      "  - Total requests:",
      response.totalRequests,
      "Total cost BRL:",
      response.totalCostBRL.toFixed(2),
    );
    debugLog(
      "  - ByClient array:",
      response.byClient
        ? `${response.byClient.length} clients`
        : "undefined (tenant mode)",
    );
