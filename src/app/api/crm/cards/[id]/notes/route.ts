...
export async function GET(
  ...
  try {
    ...
    const result = await query<any>(sqlQuery, [params.id])
...
export async function POST(
  ...
  try {
    ...
    const { data: { user } } = await (request as any).supabase?.auth.getUser() || { data: { user: null } }
    ...
    const result = await query<CRMNote>(
      `INSERT INTO crm_notes (card_id, client_id, content, created_by, is_pinned)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING *`,
      [params.id, clientId, content.trim(), user?.id || null, is_pinned]
    )
    ...
    await query(
      ...
      [clientId, params.id, user?.id || null]
    )
