...
export async function GET(request: NextRequest) {
  try {
    ...
    let gatewayQuery = supabaseServiceRole
      ...
      .select(
        "created_at, latency_ms, was_cached, error_message, provider, model_name, cost_usd, metadata",
      )
      .eq("client_id", clientId);
...
function processCostBreakdownData(data: any[]) {
  const grouped = data.reduce(
    (acc, item) => {
      ...
      acc[date].byProvider[provider] =
        (acc[date].byProvider[provider] || 0) + cost;
...
function processCostPerConversationData(
  gatewayData: any[],
  ...
  return Array.from(allDates).map((date) => {
    ...
    return {
      date,
      average:
        conversations > 0 ? Number((cost / conversations).toFixed(4)) : 0,
