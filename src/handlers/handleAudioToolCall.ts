...
export const handleAudioToolCall = async (
  ...
  if (!config.settings?.tts_enabled) {
    ...
    // Envia como texto
    try {
      const { messageId } = await sendTextMessage(phone, texto_para_audio, config)
      return { success: true, sentAsAudio: false, error: 'TTS disabled', messageId }
    } catch (error) {
      return {
        success: false,
        sentAsAudio: false,
  ...
  try {
    ...
    console.log(`[TTS] Audio generated (${audioBuffer.length} bytes, from cache: ${fromCache})`)
    ...
    if (!storageError) {
      ...
      console.log(`[TTS] Audio backed up to Supabase Storage: ${permanentAudioUrl}`)
    } else {
      console.warn('[TTS] Failed to backup audio to Supabase Storage:', storageError.message)
    }

    // 4.3 Enviar mensagem de áudio
    const { messageId } = await sendAudioMessageByMediaId(phone, mediaId, config)

    console.log(`[TTS] Audio message sent successfully! Message ID: ${messageId}`)
    ...
    try {
      ...
      return {
        success: false,
        sentAsAudio: false,
        error: textError instanceof Error ? textError.message : 'Failed to send text fallback'
      }
    }
  }
}
