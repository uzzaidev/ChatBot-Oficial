      ...
      .on(
        ...
        (payload) => {
          ...
          try {
            ...
            console.error('❌ [Realtime Conversations] Error processing clientes_whatsapp:', error)
          }
        }
      )
      ...
      .on(
        ...
        (payload) => {
          ...
          try {
            ...
            console.error('❌ [Realtime Conversations] Error processing n8n_chat_histories:', error)
          }
        }
      )
  ...
  useEffect(() => {
    ...
    const appStateListener = App.addListener('appStateChange', (state: { isActive: boolean }) => {
      if (state.isActive && channelRef.current?.state === 'closed') {
        console.log('🔄 [Realtime Conversations] App resumed, channel was closed. Reconnecting...')
        hasAttemptedRef.current = false
        setupRealtimeSubscription()
      }
    })
  ...
  useEffect(() => {
    ...
    const networkListener = Network.addListener('networkStatusChange', (status: { connected: boolean }) => {
      if (status.connected && channelRef.current?.state === 'closed') {
        console.log('🔄 [Realtime Conversations] Network reconnected, channel was closed. Reconnecting...')
        hasAttemptedRef.current = false
        setupRealtimeSubscription()
      }
    })
