...
const STATUS = {
  ...
  human_reviewed: { label: 'Revisado',  color: 'bg-blue-500/15 text-blue-600 border-blue-500/30',        dot: 'bg-blue-500',    icon: CheckCircle2 },
} as const
...
const TOOL_STATUS = {
  ...
  rejected:          { color: 'text-orange-500', bg: 'bg-orange-500/10', label: 'Rejeitado' },
  fallback_triggered:{ color: 'text-blue-500',   bg: 'bg-blue-500/10',   label: 'Fallback' },
  pending:           { color: 'text-yellow-500', bg: 'bg-yellow-500/10', label: 'Pendente' },
} as const

const TOOL_ICONS: Record<string, React.FC<{ className?: string }>> = {
  registrar_dado_cadastral:   ({ className }) => <Sparkles className={className} />,
  transferir_atendimento:     ({ className }) => <ArrowRight className={className} />,
  buscar_documento:           ({ className }) => <Search className={className} />,
  verificar_agenda:           ({ className }) => <Clock className={className} />,
  criar_evento_agenda:        ({ className }) => <CheckCircle2 className={className} />,
  ...
  return (
    <div className="metric-card flex items-start gap-3 py-4 px-5">
      ...
      <div>
        ...
        <p className="text-xl font-bold font-poppins text-foreground mt-0.5">{value}</p>
    ...
    <div className="flex flex-col items-center justify-center h-full text-center gap-4 p-8">
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1 max-w-xs">
          Clique em qualquer linha à esquerda para ver o detalhe completo — estágios do pipeline, tool calls e RAG.
        </p>
      </div>
    </div>
  )
...
type DetailTab = 'overview' | 'toolcalls' | 'rag'

function PipelineTimeline({ trace }: { trace: TraceDetail }) {
  const stages = [
    { label: 'Webhook recebido',  ts: trace.webhook_received_at, icon: Server },
    { label: 'Embedding',         ts: trace.latency_embedding_ms != null ? trace.webhook_received_at : null,
      latency: trace.latency_embedding_ms, icon: Zap },
    { label: 'RAG / Retrieval',   ts: trace.latency_retrieval_ms != null ? trace.webhook_received_at : null,
      latency: trace.latency_retrieval_ms, icon: Search },
    { label: 'Geração LLM',       ts: trace.latency_generation_ms != null ? trace.webhook_received_at : null,
      latency: trace.latency_generation_ms, icon: Bot },
        ...
        return (
          <div key={i} className="flex items-center gap-3">
            <div className={cn(
              ...
              reached ? 'bg-uzz-mint/20 text-uzz-mint' : 'bg-muted text-muted-foreground/40'
            )}>
              <Icon className="h-3.5 w-3.5" />
            </div>
            {i < stages.length - 1 && (
              <div className={cn('absolute ml-3 mt-7 h-4 w-px', reached ? 'bg-uzz-mint/30' : 'bg-border')} />
            )}
            <div className="flex-1 flex items-center justify-between">
              <span className={cn('text-xs font-medium', reached ? 'text-foreground' : 'text-muted-foreground/50')}>
                {s.label}
              </span>
              <span className="text-xs text-muted-foreground font-mono">
                {s.latency != null ? formatMs(s.latency) : s.ts ? formatFullDate(s.ts) : '—'}
              </span>
            </div>
          </div>
        )
  ...
  return (
    <div className="space-y-5 p-5">
      ...
      <div className="rounded-xl border border-border/50 bg-card/60 p-4 space-y-3">
        ...
        {trace.status === 'failed' && (trace.metadata as Record<string,unknown>)?.error && (
          <div className="rounded-lg bg-red-500/10 border border-red-500/20 p-3">
            <p className="text-xs text-red-500 font-mono">{String((trace.metadata as Record<string,unknown>).error)}</p>
          </div>
        )}
        <div className="grid grid-cols-2 gap-3 pt-1">
          {[
            { label: 'Latência total',   value: formatMs(trace.latency_total_ms) },
            { label: 'Geração LLM',      value: formatMs(trace.latency_generation_ms) },
      ...
      {/* Model + cost */}
      <div className="rounded-xl border border-border/50 bg-card/60 p-4 space-y-3">
        <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Modelo & Custo</p>
        <div className="grid grid-cols-2 gap-3">
          {[
            { label: 'Modelo',          value: trace.model_used ?? '—' },
            { label: 'Custo',           value: formatCost(trace.cost_usd) },
            { label: 'Tokens entrada',  value: trace.tokens_input?.toLocaleString('pt-BR') ?? '—' },
            { label: 'Tokens saída',    value: trace.tokens_output?.toLocaleString('pt-BR') ?? '—' },
          ].map(item => (
            <div key={item.label} className="rounded-lg bg-muted/40 px-3 py-2">
              <p className="text-[10px] text-muted-foreground uppercase tracking-wide">{item.label}</p>
      ...
      {/* Message + response */}
      <div className="rounded-xl border border-border/50 bg-card/60 p-4 space-y-3">
        <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Conversa</p>
        <div className="space-y-2">
          <div>
            <p className="text-[10px] text-muted-foreground uppercase tracking-wide mb-1">Usuário enviou</p>
            <div className="rounded-lg bg-muted/40 p-3 text-xs text-foreground leading-relaxed">
              {trace.user_message || '—'}
            </div>
          </div>
          <div>
            <p className="text-[10px] text-muted-foreground uppercase tracking-wide mb-1">Bot respondeu</p>
    ...
    return (
      <div className="flex flex-col items-center justify-center h-48 gap-3 p-6">
        ...
        <div className="text-center">
          <p className="text-sm font-medium text-muted-foreground">Nenhuma tool foi chamada</p>
          <p className="text-xs text-muted-foreground/70 mt-1">O agente respondeu diretamente, sem acionar ferramentas.</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="p-5 space-y-3">
      <p className="text-xs text-muted-foreground">
        {toolCalls.length} tool call{toolCalls.length > 1 ? 's' : ''} executada{toolCalls.length > 1 ? 's' : ''} nesta mensagem
      </p>
      {toolCalls.map((tc, i) => {
        const statusCfg = TOOL_STATUS[tc.status as keyof typeof TOOL_STATUS] ?? TOOL_STATUS.pending
        const ToolIcon = TOOL_ICONS[tc.tool_name] ?? (({ className }: { className?: string }) => <Wrench className={className} />)
          ...
          <div key={i} className="rounded-xl border border-border/50 bg-card/60 overflow-hidden">
            ...
            <div className="flex items-center gap-3 px-4 py-3 bg-muted/30">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-muted-foreground font-mono">{tc.tool_name}</p>
              </div>
              <div className="flex items-center gap-2 shrink-0">
                ...
                <span className={cn('text-xs px-2 py-0.5 rounded-full border font-medium', statusCfg.bg, statusCfg.color, 'border-current/20')}>
                  {statusCfg.label}
                </span>
                <span className="text-xs font-mono text-muted-foreground">{formatMs(tc.latency_ms)}</span>
              </div>
            </div>
            ...
            <div className="px-4 py-3 space-y-3">
              ...
              {tc.error_message && (
                <div className="rounded-lg bg-red-500/10 border border-red-500/20 p-3">
                  <p className="text-[10px] font-semibold uppercase tracking-wider text-red-500 mb-1">Erro</p>
                  <p className="text-xs text-red-400 font-mono">{tc.error_message}</p>
                </div>
              )}
            </div>
          </div>
...
function RagTab({ retrieval }: { retrieval: Retrieval | null }) {
  if (!retrieval) {
    return (
      <div className="flex flex-col items-center justify-center h-48 gap-3 p-6">
        ...
        <div className="text-center">
          <p className="text-sm font-medium text-muted-foreground">RAG não foi utilizado</p>
          <p className="text-xs text-muted-foreground/70 mt-1">Esta mensagem foi respondida sem busca na base de conhecimento.</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="p-5 space-y-4">
      {/* Config */}
      <div className="rounded-xl border border-border/50 bg-card/60 p-4">
        <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-3">Configuração da Busca</p>
      ...
      <div className="rounded-xl border border-border/50 bg-card/60 p-4">
        <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-3">
          {retrieval.chunk_ids.length} chunk{retrieval.chunk_ids.length !== 1 ? 's' : ''} retornado{retrieval.chunk_ids.length !== 1 ? 's' : ''}
        </p>
        {retrieval.chunk_ids.length === 0 ? (
          ...
          <div className="space-y-3">
            {retrieval.chunk_ids.map((id, i) => {
              ...
              const quality = score >= 0.85 ? 'text-green-500' : score >= 0.75 ? 'text-yellow-500' : 'text-orange-500'
              return (
                <div key={id} className="space-y-1">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-mono text-muted-foreground truncate max-w-[200px]">{id}</span>
                    <span className={cn('text-xs font-bold font-mono', quality)}>{score.toFixed(4)}</span>
                  </div>
                  <div className="h-1.5 rounded-full bg-muted overflow-hidden">
                    <div
                      className={cn('h-full rounded-full transition-all', score >= 0.85 ? 'bg-green-500' : score >= 0.75 ? 'bg-yellow-500' : 'bg-orange-500')}
                      style={{ width: `${pct}%` }}
                    />
                  </div>
                </div>
            ...
            <p className="text-[10px] text-muted-foreground pt-1">
              Barra mostra score relativo ao chunk mais relevante. Verde ≥ 0.85 · Amarelo ≥ 0.75 · Laranja abaixo disso.
            </p>
          </div>
        )}
      </div>
...
export function TracesClient({ initialTraceId }: TracesClientProps = {}) {
  ...
  const avgLatency = traces.length
    ? Math.round(traces.reduce((s, t) => s + (t.latency_total_ms ?? 0), 0) / traces.length)
    : 0
    ...
    <div className="flex flex-col flex-1 h-full overflow-hidden bg-background">
      ...
      <div className="border-b border-border/50 bg-card/60 px-4 py-3 shrink-0">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h1 className="text-lg font-bold font-poppins text-foreground">Traces de Mensagens</h1>
          <span className="text-xs text-muted-foreground ml-1">— rastreio completo de cada mensagem processada</span>
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
          <StatCard icon={MessageSquare} label="Mensagens hoje" value={String(totalToday)}
            sub="total processado" />
          <StatCard icon={DollarSign} label="Custo hoje" value={`$${costToday.toFixed(4)}`}
            sub="USD acumulado" />
          <StatCard icon={Clock} label="Latência média" value={formatMs(avgLatency || null)}
            sub="tempo total" />
          <StatCard icon={AlertCircle} label="Falhas" value={String(failedToday)}
            sub={`${totalToday ? Math.round((failedToday / totalToday) * 100) : 0}% do total`}
        ...
        <div className="w-full max-w-sm shrink-0 flex flex-col border-r border-border/50 bg-card/40">
          ...
          <div className="flex-1 overflow-y-auto">
            {loading ? (
              ...
              <EmptyList filtered={search.length > 0 || statusFilter !== 'all'} />
            ) : (
              filtered.map(t => (
                <TraceListItem
                  key={t.id}
              ...
              <div className="border-b border-border/40 bg-card/40 px-5 shrink-0">
                <div className="flex gap-0">
                  {([
                    ...
                    { key: 'toolcalls',  label: `Tool Calls ${selected.tool_calls.length > 0 ? `(${selected.tool_calls.length})` : ''}`,  icon: Wrench },
                    { key: 'rag',        label: 'RAG',           icon: Search },
                  ] as { key: DetailTab; label: string; icon: React.FC<{ className?: string }> }[]).map(t => (
                    <button
                      key={t.key}
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2">
        ...
        <span className={cn('inline-flex items-center gap-1 rounded-full border px-1.5 py-0.5 text-[10px] font-medium', cfg.color)}>
      ...
      <div className="flex items-center gap-3 text-[10px] text-muted-foreground">
        ...
        {trace.model_used && (
          <span className="font-mono truncate max-w-[90px]">{trace.model_used.split('/').pop()}</span>
        )}
      </div>
    </button>
  )
