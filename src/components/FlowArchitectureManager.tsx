...
export default function FlowArchitectureManager() {
  ...
  const activeBypassRoutes = nodes
    .filter((node) => {
    ...
    .map((node) => {
      ...
      return {
        node: node.name,
        disabledDeps: disabledDeps.map(
          (id) => nodes.find((n) => n.id === id)?.name || id,
        ),
        activeBypasses: activeBypasses.map(
          (id) => nodes.find((n) => n.id === id)?.name || id,
        ),
      };
    });
  ...
  const generateMermaidDiagram = useCallback(() => {
    ...
    diagram +=
      "  classDef preprocessing fill:#dbeafe,stroke:#3b82f6,stroke-width:2px\n";
    diagram +=
      "  classDef analysis fill:#fef3c7,stroke:#f59e0b,stroke-width:2px\n";
    diagram +=
      "  classDef generation fill:#d1fae5,stroke:#10b981,stroke-width:2px\n";
    diagram +=
      "  classDef output fill:#fecaca,stroke:#ef4444,stroke-width:2px\n";
    diagram +=
      "  classDef auxiliary fill:#e9d5ff,stroke:#a855f7,stroke-width:2px\n";
    diagram +=
      "  classDef disabled fill:#f3f4f6,stroke:#9ca3af,stroke-width:2px,stroke-dasharray: 5 5\n";
    ...
    nodes.forEach((node) => {
      ...
      diagram += `  class ${node.id} ${
        node.enabled ? node.category : "disabled"
      }\n`;
    });
      ...
      <CardHeader>
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
          <div>
            ...
            <CardDescription>
              Visualize e configure todos os nós (nodes) do seu chatbot
              multiagente. Clique em um nó para editar suas configurações.
            </CardDescription>
          </div>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-3">
          ...
          <Alert className="bg-orange-50 border-orange-200">
            ...
            <AlertDescription className="text-xs">
              <strong>Rotas de Bypass:</strong> Quando um node é desabilitado, o
              fluxo automaticamente usa uma rota alternativa (bypass) para o
              próximo node ativo. As linhas pontilhadas laranjas mostram quais
              caminhos alternativos serão seguidos.
            </AlertDescription>
          </Alert>
                  ...
                  {activeBypassRoutes.map((route, idx) => (
                    <li key={idx} className="text-orange-800">
                      ...
                      <span className="line-through text-gray-500">
                        {route.disabledDeps.join(", ")}
                      </span>
                      {" → "}
                      <span className="font-semibold text-orange-600">
                        {route.activeBypasses.join(", ")}
                      </span>
                    </li>
                  ))}
                </ul>
        ...
        <Alert>
          ...
          <AlertDescription className="flex items-start gap-2">
            <div>
              <strong>Como usar:</strong> Clique em qualquer nó do fluxograma
              para ver/editar suas configurações. Nós com{" "}
              <Settings className="h-3 w-3 inline" /> [Config] possuem
              configurações editáveis (prompts, temperatura, thresholds, etc).
            </div>
          </AlertDescription>
        </Alert>
      </CardContent>
            ...
            <div className="space-y-4 py-4">
              ...
              {!selectedNode.configurable && (
                <Alert>
                  <AlertCircle className="h-4 w-4" />
                  <AlertDescription>
                    Este node é essencial e não pode ser desabilitado. Ele
                    sempre será executado no fluxo.
                  </AlertDescription>
                </Alert>
              )}
                  ...
                  {(() => {
                    ...
                    const getFieldOrder = (
                      ...
                      const orderMap: Record<string, string[]> = {
                        ...
                        get_chat_history: ["max_messages"],
                        batch_messages: ["delay_seconds"],
                        get_rag_context: [
                          "enabled",
                          "similarity_threshold",
                          "max_results",
                    ...
                    const getFieldLabel = (
                      ...
                      const labelMap: Record<string, string> = {
                        // Check Continuity
                        new_conversation_threshold_hours:
                          "Threshold de Nova Conversa (Horas)",
                        greeting_for_new_customer: "Saudação - Novo Cliente",
                        greeting_for_returning_customer:
                          "Saudação - Cliente Retornando",
                    ...
                    const allKeys = Object.keys(nodeConfig).filter(
                      (key) => key !== "enabled",
                    );
                    const orderedKeys = getFieldOrder(
                      selectedNode?.id || "",
                      allKeys,
                    );
                    ...
                    return orderedKeys.map((key) => {
                      ...
                      if (
                        ...
                        const provider =
                          nodeConfig.primary_model_provider || "groq";
                        const currentModel =
                          provider === "openai"
                            ? nodeConfig.openai_model || "gpt-4o"
                            : nodeConfig.groq_model ||
                      ...
                      if (typeof value === "string" && value.length > 100) {
                        ...
                        return (
                          <div key={key} className="space-y-2">
                            ...
                            <Input
                              ...
                              type="number"
                              min={
                                isTemperature ? 0 : isThreshold ? 0 : undefined
                              }
                              max={
                                isTemperature ? 2 : isThreshold ? 1 : undefined
                              }
                        ...
                        if (isModelProvider) {
                          ...
                          const provider =
                            nodeConfig.primary_model_provider ||
                            nodeConfig.provider ||
                            "groq";
                          const groqModels = [
                            "llama-3.3-70b-versatile",
              ...
              {!selectedNode.enabled && (
                <Alert>
                  <AlertCircle className="h-4 w-4" />
                  <AlertDescription>
                    Este node está desativado. Ative-o para poder editar suas
                    configurações.
                  </AlertDescription>
                </Alert>
              )}
              ...
              {!selectedNode.hasConfig && selectedNode.enabled && (
                <Alert>
                  <AlertCircle className="h-4 w-4" />
                  <AlertDescription>
                    Este node não possui configurações editáveis. Ele faz parte
                    do pipeline padrão do chatbot.
                  </AlertDescription>
                </Alert>
              )}
            </div>
