...
export const UserCreateSchema = z.object({
  ...
  client_id: z.string().uuid("Client ID deve ser um UUID válido").optional(),
...
export const SecretUpdateSchema = z.object({
  key: z.enum(
    [
      ...
      "openai_api_key",
      "openai_admin_key", // Admin key for billing/usage APIs (scope: api.usage.read)
      "groq_api_key",
    ],
    {
...
export const BotConfigSchema = z.object({
  ...
  auto_reply: z.boolean().optional().default(true),
  ...
  // RAG config
  rag_enabled: z.boolean().optional().default(false),
  ...
  rag_collection_name: z
    ...
    .regex(
      /^[a-z0-9_-]+$/,
      "Collection name deve conter apenas letras minúsculas, números, _ e -",
    )
    .optional(),

  rag_top_k: z.number().int().min(1).max(20).optional().default(5),
 ...
 */
export const BotConfigUpdateSchema = BotConfigSchema.partial().omit({
  client_id: true,
});
