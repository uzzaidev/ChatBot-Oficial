...
const getConnectionString = (): string => {
  ...
  const host =
    process.env.POSTGRES_HOST || "db.jhodhxvvhohygijqcxbo.supabase.co";
...
export const getPool = (): Pool => {
  ...
  pool = new Pool({
    ...
    min: 1, // Mantém ao menos 1 conexão warm (reduz latência de cold-start)
    idleTimeoutMillis: 30000, // 30s: mantém conexões warm entre requests
...
const validateConnection = async (testPool: Pool): Promise<boolean> => {
  ...
  try {
    ...
    client = await Promise.race([testPool.connect(), timeoutPromise]);
