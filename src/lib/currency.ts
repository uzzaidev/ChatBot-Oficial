...
export const getExchangeRate = async (
  ...
  const debug = process.env.CURRENCY_DEBUG === "true";
  ...
  if (cached && Date.now() - cached.timestamp < CACHE_TTL_MS) {
    if (debug) {
      console.log(
        `[Currency] Using cached rate: ${from} → ${to} = ${cached.rate}`,
      );
    }
    return cached.rate;
  }
...
export const clearExchangeRateCache = (): void => {
  ...
  if (process.env.CURRENCY_DEBUG === "true") {
...
export const getCacheStatus = (): {
  ...
  const entries = Array.from(exchangeRateCache.entries()).map(
    ([pair, data]) => {
