...
export const getClientByWABAId = async (
  wabaId: string,
  ...
  try {
    ...
    if (cached) {
      ...
      console.log(
        `[WABA Lookup] Cache hit for WABA ${wabaId} → client ${parsed.id}`,
      );
      return parsed;
    }
  } catch (cacheError) {
    console.warn(
      "[WABA Lookup] Redis cache unavailable, falling back to DB:",
      cacheError,
    );
  }
  ...
  try {
    ...
    console.error(
      `[WABA Lookup] getClientConfig threw for client ${clientId}:`,
  ...
  if (!config) {
    console.warn(
      `[WABA Lookup] getClientConfig returned null for client ${clientId}`,
    );
    return null;
  }

  console.log(
    `[WABA Lookup] DB hit for WABA ${wabaId} → client ${config.id} (${config.name})`,
  );
