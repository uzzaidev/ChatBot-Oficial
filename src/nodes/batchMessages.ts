...
export const batchMessages = async (
  ...
  try {
    ...
    if (delaySeconds === undefined) {
      ...
      const delayConfig = await getBotConfig(
        clientId,
    ...
    if (!lockAcquired) {
      ...
      console.log(
        `[batchMessages] Lock exists for ${phone}, skipping processing`,
      );
      return "";
    }
    ...
    if (lastProcessedStr) {
      ...
      if (timeSinceProcessed < 60000) {
        ...
        if (pendingMessages.length === 0) {
          ...
          console.log(
            `[batchMessages] Recently processed for ${phone} (${timeSinceProcessed}ms ago) and no new messages, skipping`,
          );
        ...
        console.log(
          `[batchMessages] Recently processed for ${phone} (${timeSinceProcessed}ms ago) but has ${pendingMessages.length} new messages, continuing`,
        );
      }
    }

    console.log(
      `[batchMessages] Lock acquired for ${phone}, waiting ${delaySeconds}s`,
    );
    ...
    while (restartCount < MAX_RESTARTS) {
      ...
      if (lastMessageTimestamp) {
        const timeSinceLastMessage =
          Date.now() - parseInt(lastMessageTimestamp, 10);
        ...
        if (timeSinceLastMessage < BATCH_DELAY_MS) {
          restartCount++;
          console.log(
            `[batchMessages] Debounce reset for ${phone} (${timeSinceLastMessage}ms < ${BATCH_DELAY_MS}ms), restarting wait (attempt ${restartCount}/${MAX_RESTARTS})`,
          );
    ...
    if (restartCount >= MAX_RESTARTS) {
      console.log(
        `[batchMessages] Max restarts (${MAX_RESTARTS}) reached for ${phone}, processing anyway`,
      );
    }
    ...
    console.log(
      `[batchMessages] Processing ${parsedMessages.length} messages for ${phone}`,
    );
    ...
    try {
      ...
      console.error(
        `[batchMessages] Failed to cleanup lock for ${phone}:`,
        cleanupError,
      );
    }
