...
export const checkContinuity = async (
  ...
  try {
    ...
    const thresholdHours = await getBotConfig(
      clientId,
      "continuity:new_conversation_threshold_hours",
    );
    const thresholdValue =
      thresholdHours !== null ? Number(thresholdHours) : 24; // Default 24 hours
    ...
    const finalGreeting =
      greetingInstruction !== null ? String(greetingInstruction) : ""; // No default greeting — configure via dashboard if needed
