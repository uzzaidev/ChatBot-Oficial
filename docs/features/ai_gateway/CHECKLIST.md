...
- [ ] `src/app/dashboard/ai-gateway/page.tsx` (ADMIN ONLY - Config Global)
  ...
  - [ ] **Aviso: "Keys compartilhadas por todos os clientes"**

- [ ] `src/app/dashboard/ai-gateway/metrics/page.tsx` (ADMIN - Métricas Agregadas)
  - [ ] Cards: Total Requests (todos clientes), Cost BRL agregado, Cache Hit Rate global
  - [ ] Chart: Usage by Client (top 10)
  - [ ] Chart: Usage by Provider
  - [ ] Chart: Cost Over Time (agregado)
  - [ ] Table: Top Clients by Usage
...
- [ ] `src/components/ModelSelector.tsx`
  - [ ] Fetch de /api/ai-gateway/models
  - [ ] Filtro por capabilities
  - [ ] Badge com provider
  - [ ] Pricing display

- [ ] `src/components/GatewayMetricsDashboard.tsx`
  - [ ] Hook `useGatewayMetrics()`
  - [ ] Render cards
...
- [ ] **NOVO: `src/components/BudgetConfiguration.tsx`**
  - [ ] Budget type selector
  - [ ] Budget limit input com validação
  - [ ] Period selector
  - [ ] Alert thresholds checkboxes
  - [ ] Auto-pause toggle
  - [ ] Save/Cancel buttons

- [ ] **NOVO: `src/components/BudgetProgressBar.tsx`**
  - [ ] Progress bar com cores (green/yellow/orange/red)
  - [ ] Current usage / limit display
  - [ ] Percentage calculation
  - [ ] Days remaining in period
  - [ ] Projeção de uso (estimativa)

- [ ] **NOVO: `src/components/LatencyChart.tsx`**
...
- [ ] `src/hooks/useGatewayMetrics.ts`
  - [ ] Fetch de /api/ai-gateway/metrics
  - [ ] State management (loading, error, data)
  - [ ] Refetch function
  - [ ] Cache SWR/React Query

- [ ] **NOVO: `src/hooks/useBudget.ts`**
  - [ ] Fetch de /api/budget/config
  - [ ] Fetch de /api/budget/current-usage
  - [ ] Real-time updates (polling every 5min)
...
- [ ] **`src/app/api/ai-gateway/config/route.ts`** (ADMIN ONLY)
  - [ ] GET - buscar `shared_gateway_config` (única)
  - [ ] POST - atualizar shared config (Vault + DB)
  - [ ] DELETE - limpar shared config
  - [ ] **Auth check: admin only**
  - [ ] Error handling

- [ ] `src/app/api/ai-gateway/models/route.ts`
  - [ ] GET - listar modelos ativos
  - [ ] Query param: capabilities filter
  - [ ] Ordenação por provider/model_name

- [ ] **`src/app/api/ai-gateway/metrics/route.ts`** (ADMIN - Agregado)
  - [ ] GET - agregar métricas de TODOS os clientes
  - [ ] Query params: startDate, endDate
  - [ ] Calcular: totalRequests (all), costBRL (all), cacheHitRate (global)
  - [ ] Calcular usage by client (top 10)
  - [ ] Calcular latency P50/P95/P99 (global)
  - [ ] Retornar arrays para charts

- [ ] **NOVO: `src/app/api/budget/config/route.ts`**
  - [ ] GET - buscar budget do cliente atual
  - [ ] POST - criar/atualizar budget
  - [ ] DELETE - resetar para default do plano
  - [ ] Validação: budget_limit > 0
  - [ ] RLS policy check

- [ ] **NOVO: `src/app/api/budget/current-usage/route.ts`**
  - [ ] GET - calcular uso atual do período (filtrado por `client_id`)
  - [ ] Query params: period (optional override)
  - [ ] Agregar de `gateway_usage_logs WHERE client_id = ...`
  - [ ] Retornar: usage, limit, percentage, remaining
  - [ ] Calcular projeção fim do período
