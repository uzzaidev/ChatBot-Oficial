  ...
  return (
    <Card
      ...
      className={cn(
        ...
        card.auto_status === 'awaiting_client' && 'border-l-4 border-l-uzz-mint'
      )}
      onClick={onClick}
      {...attributes}
      {...listeners}
    >
      <CardContent className="p-3 space-y-3">
        ...
        {/* Tags */}
        {cardTags.length > 0 && (
          <CardTagList tags={cardTags} maxVisible={3} />
        )}
...
const STATUS_CONFIG: Record<AutoStatus, {
  label: string
...
const TAG_COLORS: Record<string, string> = {
  ...
  default: 'bg-secondary/20 text-secondary-foreground border-secondary/30 hover:bg-secondary/30',
}

export function CardTagList({ tags, maxVisible = 5, onRemove }: CardTagListProps) {
  ...
  return (
    <div className="flex flex-wrap gap-1">
      ...
      {hiddenCount > 0 && (
        <Badge variant="outline" className="text-xs px-1.5 py-0 font-normal bg-muted">
          +{hiddenCount}
        </Badge>
      )}
    </div>
          ...
          <ScrollArea className="flex-1">
            <TabsContent value="info" className="p-4 space-y-6 m-0">
              ...
              {card.estimated_value && (
                <div>
                  <h4 className="text-sm font-medium text-foreground mb-2">Valor Estimado</h4>
...
export function ColumnHeader({ name, count, icon = 'Users' }: ColumnHeaderProps) {
...
export default function CRMPage() {
  ...
  // Hooks
  const { columns, createColumn, updateColumn, deleteColumn, reorderColumns } = useCRMColumns(clientId)
  const { cards, moveCard, refetch: refetchCards } = useCRMCards(clientId, filters)
...
export function useCRMCards(clientId: string | null, filters: CRMCardsFilters = {}) {
  ...
  }, [clientId, filters.search, filters.autoStatus, filters.tagId, filters.assignedTo])
  ...
  const moveCard = async (cardId: string, targetColumnId: string, position?: number) => {
    ...
    setCards(prev => prev.map(c =>
      c.id === cardId
        ? { ...c, column_id: targetColumnId, moved_to_column_at: new Date().toISOString() }
        : c
    ))
...
export interface CRMActivityLog {
  ...
  client_id: string
  card_id: string
  activity_type: 'column_move' | 'tag_add' | 'tag_remove' | 'note_add' | 'assigned' | 'status_change' | 'value_change' | 'created'
  description: string | null
...
export interface CRMAutomationRule {
  ...
  name: string
  description: string | null
  trigger_type: 'time_in_column' | 'no_response' | 'message_received' | 'tag_added'
  trigger_config: Record<string, unknown>
  action_type: 'move_column' | 'add_tag' | 'send_message' | 'assign_user' | 'notify'
...
export async function updateCRMCardStatus({
  ...
  const newStatus = direction === 'incoming'
    ? 'awaiting_attendant'
    : 'awaiting_client'
...
- [ ] Migration: crm_columns, crm_cards, crm_tags, crm_card_tags
- [ ] RLS policies
- [ ] Seed colunas padrão (Novo, Qualificando, Proposta, Fechado)
- [ ] API: CRUD colunas
- [ ] API: CRUD cards + move
- [ ] API: CRUD tags
- [ ] Componente: KanbanCard (reutilizável)
- [ ] Componente: KanbanColumn
- [ ] Componente: KanbanBoard com drag-and-drop
- [ ] Componente: CardDetailPanel básico
- [ ] Hooks: useCRMColumns, useCRMCards, useCRMTags
- [ ] Página: /dashboard/crm
- [ ] Mobile: Tabs por coluna
- [ ] Link na navegação do dashboard

### Fase 2: v1.0 (2-3 semanas)

- [ ] Migration: crm_notes, crm_activity_log, scheduled_messages
- [ ] API: CRUD notas
- [ ] API: Activity log
- [ ] API: Scheduled messages
- [ ] Componente: CardNotes
- [ ] Componente: CardTimeline
- [ ] Componente: ScheduleMessageDialog
- [ ] Node: updateCRMCardStatus
- [ ] Integração: Auto-status no chatbotFlow
- [ ] Cron: Processador de mensagens agendadas
- [ ] Filtros: por auto-status, tag, responsável

### Fase 3: v2.0 (3-4 semanas)

- [ ] Migration: lead_sources, crm_automation_rules
- [ ] API: Lead sources
- [ ] API: Analytics (funnel, sources, response time)
- [ ] API: Automation rules
- [ ] Node: captureLeadSource
- [ ] Integração: Captura de referral Meta Ads no webhook
- [ ] Auto-tagging de leads de anúncios
- [ ] Componente: AutomationRulesPanel
